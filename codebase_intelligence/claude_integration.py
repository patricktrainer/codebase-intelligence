        ...
        try:
            ...
            for line in clean_output.strip().split("\n"):
                if line.strip():
                    try:
                        ...
                        LOGGER.warning(
                            f"Could not decode JSON line from Claude output: {line}"
                        )
                        continue
            ...
            if not last_result_obj:
                ...
                if output.strip():
                    ...
                    return ClaudeCodeResult(
                        success=False,
                        error="No 'result' type object found in Claude output.",
                        output=output,
                    )

            if last_result_obj.get("is_error"):
                return ClaudeCodeResult(
                    success=False, error=last_result_obj.get("result", "Unknown error")
                )
            ...
            if "```json" in final_output:
                ...
                if match:
                    ...
                    final_output = (
                        final_output.replace("```json", "").replace("```", "").strip()
                    )
